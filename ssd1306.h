...
#define SSD1306_I2C_ADDR    0x78//device I2C address	0x44 write	0x45 read
#endif
...
typedef enum OLED_status_t {//состояние устройства
	IOLED_Init,		//устройство не настроено
	OLED_OK,		//устройство готово к опросу
	OLED_Faulth	//устройство неисправно
} OLED_status;

/*	состояние обмена данными с устройством, использовать для завершения функции работы с устройством */
typedef enum OLED_Connect_Status_t {
	OLED_Processing, //выполняется работа с устройством: обмен данными, обработка результатов
	OLED_Complite	//работа с устройством завершена, данные считаны/записаны корректно
} OLED_Connect_Status;
